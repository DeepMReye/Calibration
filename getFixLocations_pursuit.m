...
mov_amp = repmat(perms(mov_amp), 1, numel(settings.pursuit.angles)/size(perms(1:numel(mov_amp)),1))';
mov_amp = mov_amp(:)';
...
cTrial = 2; stuck = 0;
while cTrial <= numel(mov_angles)+1
...
if size(settings.pursuit.xy,1) == numel(mov_angles)+1
    valid = 1;
    for cTrial = 2:numel(mov_angles)+1
        settings.pursuit.xy_trials_pursuit{cTrial-1}(1,:) = linspace(settings.pursuit.xy(cTrial-1,1), settings.pursuit.xy(cTrial,1), settings.pursuit.mov_dur*settings.scr.hz+1);
        settings.pursuit.xy_trials_pursuit{cTrial-1}(2,:) = linspace(settings.pursuit.xy(cTrial-1,2), settings.pursuit.xy(cTrial,2), settings.pursuit.mov_dur*settings.scr.hz+1);
    end
end
...
settings.pursuit.xy_trials_pursuit = [{zeros(settings.pursuit.mov_dur*settings.scr.hz,2)}, settings.pursuit.xy_trials_pursuit];
